namespace ShorsAzure {
    ...
    operation ShorsFactor() : Unit {
        ...
        //Message("GCD: "+ IntAsString(gcd));
         use Qubits = Qubit[7] { //THE "circuit". All operations below should be done in here.
            ...
            repeat{
                ...
                if (topPhase != IntAsDouble(0)){
                    set guessOne = GreatestCommonDivisorI(a^(r/2)-1, N);
                    set guessTwo = GreatestCommonDivisorI(a^(r/2)+1, N);
                    Message("Guessed Factors: " + IntAsString(guessOne) + " and " + IntAsString(guessTwo) + "\n");

                    if (guessOne != 1 and (N % guessOne) == 0){
                        Message("***** Factor: " + IntAsString(guessOne) + " *****\n");
                        set factor_found = true;
                    }
                    if (guessTwo != 1 and (N % guessTwo) == 0){
